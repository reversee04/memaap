...
exports.AuthService = exports.ValidationError = exports.AuthError = void 0;
...
class AuthService {
     ...
     */
    static async register(name, phone, email, password, role) {
        ...
        try {
            ...
            // Insert user
            const result = await client.query(`INSERT INTO users (name, phone, email, password_hash, role, created_at, updated_at) 
         VALUES ($1, $2, $3, $4, $5, NOW(), NOW()) 
         RETURNING id, name, phone, email, role, created_at, updated_at`, [name, phone, email, hashedPassword, role]);
    ...
    static async login(phone, password) {
        ...
        try {
            ...
            // Find user by phone
            const result = await client.query(`SELECT id, name, phone, email, password_hash, role, created_at, updated_at 
    ...
    static async refreshToken(oldToken) {
        ...
        try {
            ...
            const refreshTokenResult = await client.query(`SELECT token, expires_at FROM refresh_tokens 
         WHERE user_id = $1 AND expires_at > NOW() LIMIT 1`, [decoded.userId]);
            if (refreshTokenResult.rows.length === 0) {
                throw new AuthError('No valid refresh token found');
            }
            // Get user data
            const userResult = await client.query(`SELECT id, name, phone, email, role, created_at, updated_at 
    ...
    static async logout(userId, token) {
        ...
        try {
            ...
            await client.query('INSERT INTO revoked_tokens (token, revoked_at) VALUES ($1, NOW())', [token]);
    ...
    static async isTokenRevoked(token) {
        const result = await pool.query('SELECT token FROM revoked_tokens WHERE token = $1 LIMIT 1', [token]);
    ...
    static async generateTokens(user) {
        ...
        const token = jsonwebtoken_1.default.sign(tokenPayload, privateKey, {
            algorithm: 'RS256',
            expiresIn: '15m',
        ...
        const refreshToken = jsonwebtoken_1.default.sign(refreshTokenPayload, privateKey, {
            algorithm: 'RS256',
            expiresIn: '7d',
            issuer: 'memaap-backend',
            audience: 'memaap-mobile'
     ...
     */
    static async storeRefreshToken(client, userId, refreshToken) {
     ...
     * @throws ValidationError if validation fails
     */
    static validateRegistrationInput(name, phone, email, password, role) {
